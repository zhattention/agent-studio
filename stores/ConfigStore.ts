...
export class ConfigStore {
  ...
  updateAgentsFromNodes = (agentsData: any[]) => {
    ...
    console.log('[ConfigStore] 节点和边总数:', { 
    ...
    console.log('[ConfigStore] 找到团队节点:', { 
      count: teamNodes.length, 
      teamNames: teamNodes.map(n => n.data.name) 
    });
    
    console.log('[ConfigStore] 找到Agent节点:', { 
    ...
    console.log('[ConfigStore] 构建节点连接关系:', { 
      outConnectionCount: outConnections.size,
      inConnectionCount: inConnections.size
    });
    ...
    const buildTeamConfig = (teamNodeId: string, depth = 0, processedTeams = new Set<string>()) => {
      ...
      if (processedTeams.has(teamNodeId)) {
        console.log(`[ConfigStore] [${'  '.repeat(depth)}] 团队节点已处理过，跳过:`, teamNodeId);
        return null;
      }
      ...
      if (!teamNode) {
        console.log(`[ConfigStore] [${'  '.repeat(depth)}] 未找到团队节点:`, teamNodeId);
        return null;
      }
      
      console.log(`[ConfigStore] [${'  '.repeat(depth)}] 开始构建团队 "${teamNode.data.name}" 配置`);
      
      // 找出团队关联的所有节点（按链式关系查找）
      const allConnected = findAllConnectedNodes(teamNodeId, true);
      
      console.log(`[ConfigStore] [${'  '.repeat(depth)}] 团队 "${teamNode.data.name}" 链式连接了 ${allConnected.agentNodes.length} 个agent节点, ${allConnected.teamNodes.length} 个team节点`);
      ...
      console.log(`[ConfigStore] [${'  '.repeat(depth)}] 团队 "${teamNode.data.name}" 直接连接了 ${directAgentNodes.length} 个agent节点`);
      ...
      console.log(`[ConfigStore] [${'  '.repeat(depth)}] 团队 "${teamNode.data.name}" 直接连接了 ${directTeamNodes.length} 个team节点`);
      ...
      console.log(`[ConfigStore] [${'  '.repeat(depth)}] 团队 "${teamNode.data.name}" 链式排序后有 ${orderedAgents.length} 个agent节点`);
      console.log(`[ConfigStore] [${'  '.repeat(depth)}] 发现 ${teamConnections.size} 个agent到team的连接`);
      
      // 7. 按顺序处理每个agent节点
      for (const agentNode of orderedAgents) {
        console.log(`[ConfigStore] [${'  '.repeat(depth)}] 处理agent节点: "${agentNode.data.name}" (${agentNode.id})`);
        ...
        if (connectedTeamId) {
          ...
          if (subTeamNode) {
            console.log(`[ConfigStore] [${'  '.repeat(depth)}] Agent "${agentNode.data.name}" 连接到子团队: "${subTeamNode.data.name}"`);
            
            agentConfig.team_call = subTeamNode.data.name;
            agentConfig.team_cfg = buildTeamConfig(subTeamNode.id, depth + 1, processedTeams);
            
            console.log(`[ConfigStore] [${'  '.repeat(depth)}] 完成agent "${agentNode.data.name}" 的子团队配置构建`);
          }
        }
      ...
      for (const subTeamNode of directTeamNodes) {
        ...
        if (!alreadyProcessed) {
          console.log(`[ConfigStore] [${'  '.repeat(depth)}] 处理直接子团队: "${subTeamNode.data.name}"`);
          
          const subTeamConfig = buildTeamConfig(subTeamNode.id, depth + 1, processedTeams);
          if (subTeamConfig) {
            // 创建一个虚拟agent，将子团队嵌套在其中
            console.log(`[ConfigStore] [${'  '.repeat(depth)}] 创建虚拟agent嵌套子团队 "${subTeamNode.data.name}"`);
      ...
      const teamConfig = {
        ...
        team_type: teamNode.data.team_type || "round_robin",
        team_prompt: teamNode.data.team_prompt || "",
        agents: agents,
        duration: teamNode.data.duration || 0
      };
      
      console.log(`[ConfigStore] [${'  '.repeat(depth)}] 完成团队 "${teamNode.data.name}" 配置构建, 包含 ${agents.length} 个agents`);
      return teamConfig;
    };
    
    // 9. 找到根团队节点（没有入边的团队节点）
    const rootTeamNodes = teamNodes.filter(node => {
      const incomingEdges = inConnections.get(node.id) || [];
      // 只有agent节点指向团队的情况，不算作入边
      const hasTeamIncoming = incomingEdges.some(sourceId => {
        const sourceNode = allNodes.find(n => n.id === sourceId);
        return sourceNode && sourceNode.type === 'team';
      });
      return !hasTeamIncoming;
    });
    
    console.log('[ConfigStore] 找到根团队节点:', { 
      count: rootTeamNodes.length, 
      rootTeamNames: rootTeamNodes.map(n => n.data.name) 
    });
    
    // 如果有根团队节点，使用它构建配置
    if (rootTeamNodes.length > 0) {
      console.log('[ConfigStore] 使用根团队节点构建配置:', rootTeamNodes[0].data.name);
      
      const rootTeamConfig = buildTeamConfig(rootTeamNodes[0].id);
      if (rootTeamConfig) {
        this.teamConfig = {
          ...this.teamConfig,
          ...rootTeamConfig
        ...
        console.log('[ConfigStore] 成功从节点图构建团队配置树', { 
    ...
    // 10. 如果没有找到根团队，回退到默认行为 - 使用所有agent节点
    console.warn('[ConfigStore] 无法找到根团队节点，使用默认方法更新配置');
    this.teamConfig = {
      ...this.teamConfig,
      agents: agentsData
    };
    
    console.log('[ConfigStore] 使用默认方法完成配置更新', {
      teamName: this.teamConfig.name,
      agentsCount: this.teamConfig.agents.length
    });
  };
  ...
  cleanConfigForSave = (config: any): any => {
    ...
    if (Array.isArray(config)) {
      return config.map(item => this.cleanConfigForSave(item));
    }
    ...
    for (const key in config) {
      if (excludeFields.includes(key)) {
        console.log(`[ConfigStore] 清理配置 - 移除字段: ${key}`);
        continue;
      }
      
      // 递归清理子对象
      cleanedConfig[key] = this.cleanConfigForSave(config[key]);
    }
  ...
  saveTeamConfig = async (): Promise<boolean> => {
    try {
      ...
      // 清理配置，移除不必要的字段
      const cleanedConfig = this.cleanConfigForSave(this.teamConfig);
