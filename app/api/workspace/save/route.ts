...
export async function POST(request: Request) {
  try {
    console.log('Handling workspace save request...');
    ...
    console.log('Request body received:', {
      name: body.name,
      dataExists: !!body.data,
      nodeCount: body.data?.nodes?.length
    });
    ...
    const workspaceDir = path.join(process.cwd(), 'agent-data', 'workspace', user.username, name);
